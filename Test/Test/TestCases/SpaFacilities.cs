...
namespace Test.TestCases
{
    public class SpaFacilities
    {
        public static void FilterHotels(IWebDriver driver)
        {
            ...
            // Filter by Spa Facilities
            Thread.Sleep(2000);
            trivagoHotelPage.FilterBySpaFacilities();

            Common.CheckExpectedResults(driver, Constant.SpaFacilities);

            // Assert if The River Lee Hotel is shown on the results and Jurys Inn Hotel is not
            Assert.IsTrue(Common.TheRiverLee, "The River Lee Hotel was not on the page for Spa Facilites");
            Assert.IsFalse(Common.JurysInn, "Jurys Inn Hotel was on the page for Spa Facilites");
        }
    }
}
